...
// Agente autocomplete options
const agenteOptions = ref([])

async function fetchAgentes() {
  try {
    const response = await fetch(
      route('relatorios.exportacao.comissao_exportacao.agentes'),
      { headers: { 'X-Requested-With': 'XMLHttpRequest' } }
    )
    if (response.ok) {
      agenteOptions.value = await response.json()
    }
  } catch {
    agenteOptions.value = []
  }
...
onMounted(() => fetchAgentes())

const lookups = computed(() => ({
  nom_agente: agenteOptions.value,
}))
...
async function handleSearch(values) {
  ...
  try {
    const response = await fetch(
      `${route('relatorios.exportacao.comissao_exportacao.pesquisar')}?${params.toString()}`,
      { headers: { 'X-Requested-With': 'XMLHttpRequest' } }
    )
  ...
  <ReportPageLayout :title="title" :section="section">
    ...
    <div v-if="!searching && gridData.length > 0" class="mt-6">
      <GenericDataGrid
        ...
        :data="gridData"
        :page-size="25"
        export-filename="comissao-exportacao"
      />
    </div>
