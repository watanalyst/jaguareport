@php
    ...
    // --- Build item description map (cod_item → den_item_int) ---
    $itemDescriptions = [];
    foreach ($items as $item) {
        $itemDescriptions[trim($item->cod_item ?? '')] = trim($item->den_item_int ?? '');
    }
@endphp
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <title>Expiry and Lot Number Certificate {{ trim($h->proforma) }}-{{ trim($h->embarque) }}</title>
    <style>
...
<table cellspacing="0" cellpadding="2">
    <tr>
        <th align="center" style="font-size: 16px; font-weight: bold;">EXPIRY AND LOT NUMBER CERTIFICATE</th>
    </tr>
    <tr>
...
{{-- ══════════════════════════════════════════════════════════════
     SECTION 5: ITEMS TABLE (SUMMARY GRID)
     ══════════════════════════════════════════════════════════════ --}}
<table cellspacing="0" cellpadding="3">
    <tr>
        <th class="th-header b-t b-b b-l">CARTONS</th>
        <th class="th-header b-t b-b b-l">NET WEIGHT (KG)</th>
        <th class="th-header b-t b-b b-l">GROSS WEIGHT (KG)</th>
...
{{-- ══════════════════════════════════════════════════════════════
     SECTION 6: PACKING DETAIL PER ITEM
     ══════════════════════════════════════════════════════════════ --}}
@if(! empty($packingDetail))
    @foreach($packingDetail as $codItem => $details)
    @php
        $denItem = $itemDescriptions[$codItem] ?? $codItem;
    @endphp
    <table cellspacing="0" cellpadding="3" style="width: 60%; margin-bottom: 5px;">
        <tr>
            <th colspan="5" align="center" class="b-t b-l b-r">{{ $codItem }} - {{ $denItem }}</th>
        </tr>
        <tr>
            ...
            <th class="th-header b-t b-b b-l" style="width:15%;">CARTONS</th>
            <th class="th-header b-t b-b b-l b-r" style="width:20%;">NET WEIGHT</th>
            <th class="th-header b-t b-b b-r" style="width:15%;">LOTS</th>
        </tr>
        @php
            $totCartons   = 0;
            $totNetWeight = 0;
        ...
        @php
            $totCartons   += (float) ($d->cartons ?? 0);
            $totNetWeight += (float) ($d->net_weight ?? 0);
        @endphp
        <tr>
            <td align="center" class="b-b b-l">{{ $d->production_date ?? '' }}</td>
            <td align="center" class="b-b b-l">{{ $d->expiry_date ?? '' }}</td>
            <td align="right" class="b-b b-l">{{ number_format((float)($d->cartons ?? 0), 0, '.', ',') }}</td>
            <td align="right" class="b-b b-l b-r">{{ number_format((float)($d->net_weight ?? 0), 2, '.', ',') }}</td>
            <td align="center" class="b-b b-r">{{ $d->lots ?? '' }}</td>
        </tr>
        @endforeach
        <tr>
            <th class="b-b b-l"></th>
            <th align="right" class="b-b b-l">TOTAL:</th>
            <td align="right" class="b-b b-l">{{ number_format($totCartons, 0, '.', ',') }}</td>
            <td align="right" class="b-b b-l b-r">{{ number_format($totNetWeight, 2, '.', ',') }}</td>
            <td class="b-b b-r"></td>
        </tr>
    </table>
    @endforeach
...
{{-- ══════════════════════════════════════════════════════════════
     SECTION 8: FOOTER (end of document)
     ══════════════════════════════════════════════════════════════ --}}
<table cellspacing="0" cellpadding="3">
    <tr>
