...
use App\Http\Requests\Relatorios\Exportacao\CambioPeriodoPesquisarRequest;
use App\Services\Reports\CambioPeriodoService;
use Inertia\Inertia;

class CambioPeriodoController extends Controller
{
    public function index(CambioPeriodoService $service)
    {
        try {
        ...
        $columns = [
            ...
            ['key' => 'dat_cambio',      'label' => 'Dat Câmbio',     'sortable' => true, 'type' => 'date'],
        ...
        return Inertia::render('Relatorios/Exportacao/CambioPeriodo/Index', [
            'title'    => 'Câmbio Período',
            'section'  => 'Exportação',
            'filters'  => config('reports.exportacao.financeiro_exp.children.cambio_periodo.filters'),
            'empresas' => $empresas,
            'columns'  => $columns,
        ]);
    }

    public function pesquisar(CambioPeriodoPesquisarRequest $request, CambioPeriodoService $service)
    {
        try {
